#!/bin/bash
# =============================================================================
# WireGuard + WARP + AmneziaWG 生產級部署腳本 v4.1 (修復版)
...
# 回退選項
readonly FALLBACK_TO_USERSPACE="${FALLBACK_TO_USERSPACE:-true}"
...
install_packages() {
    ...
    local packages=(
        ...
        python3 python3-pip python3-venv pipx systemd
        wireguard wireguard-tools
    )
    
    apt-get install -y "${packages[@]}" || error_exit "基礎套件安裝失敗"
    
    # 修復 Python 模組安裝（Ubuntu 24.04 相容性）
    log "INFO" "安裝 Python 模組..."
    if command -v pipx >/dev/null 2>&1; then
        # 使用系統套件安裝 Python 模組
        apt-get install -y python3-requests python3-psutil 2>/dev/null || \
        pip3 install --break-system-packages requests psutil 2>/dev/null || \
        log "WARN" "Python 模組安裝失敗，繼續執行"
    else
        pip3 install --break-system-packages requests psutil 2>/dev/null || \
        log "WARN" "Python 模組安裝失敗，繼續執行"
    fi
...
# ===================== 安裝 AmneziaWG =====================
install_amneziawg() {
    log "INFO" "嘗試安裝 AmneziaWG..."
    
    # 首先嘗試安裝 linux-source 套件
    local kernel_version
    kernel_version=$(uname -r | cut -d'-' -f1-2)
    
    log "INFO" "安裝核心原始碼套件..."
    apt-get install -y linux-source-${kernel_version} 2>/dev/null || \
    apt-get install -y linux-source 2>/dev/null || \
    log "WARN" "無法安裝核心原始碼套件"
    
    # 解壓核心原始碼（如果存在）
    if [[ -f /usr/src/linux-source-*.tar.bz2 ]]; then
        cd /usr/src
        tar -xf linux-source-*.tar.bz2 2>/dev/null || true
        cd - >/dev/null
    fi
    ...
    if apt-get install -y amneziawg amneziawg-tools; then
        ...
        if modprobe amneziawg 2>/dev/null && lsmod | grep -q "amneziawg"; then
            ...
            log "WARN" "AmneziaWG 核心模組載入失敗，嘗試修復..."
            
            # 嘗試手動編譯 DKMS
            if dkms status | grep -q amneziawg; then
                log "INFO" "重新編譯 AmneziaWG DKMS 模組..."
                dkms remove amneziawg/1.0.0 --all 2>/dev/null || true
                dkms install amneziawg/1.0.0 2>/dev/null || true
                
                if modprobe amneziawg 2>/dev/null && lsmod | grep -q "amneziawg"; then
                    log "INFO" "AmneziaWG DKMS 修復成功"
                    echo "amneziawg" >> /etc/modules-load.d/amneziawg.conf
                    return 0
                fi
            fi
    ...
    # 如果 AmneziaWG 安裝失敗，檢查是否啟用回退
    if [[ "$FALLBACK_TO_USERSPACE" == "true" ]]; then
        log "WARN" "AmneziaWG 核心模組安裝失敗，回退到 userspace 模式"
        log "INFO" "將使用標準 WireGuard + userspace AmneziaWG 工具"
        
        # 確保標準 WireGuard 已安裝
        if ! command -v wg >/dev/null 2>&1; then
            apt-get install -y wireguard wireguard-tools || error_exit "無法安裝標準 WireGuard"
        fi
        
        # 安裝 amneziawg-go (userspace 實作)
        install_amneziawg_userspace
        return 0
    else
        error_exit "AmneziaWG 安裝失敗且未啟用回退模式"
    fi
}

# ===================== 安裝 AmneziaWG Userspace 版本 =====================
install_amneziawg_userspace() {
    log "INFO" "安裝 AmneziaWG userspace 版本..."
    
    # 下載最新的 amneziawg-go
    local awg_go_url
    awg_go_url=$(curl -s https://api.github.com/repos/amnezia-vpn/amneziawg-go/releases/latest | \
        jq -r '.assets[] | select(.name | test("linux.*amd64")) | .browser_download_url' | head -1)
    
    if [[ -n "$awg_go_url" ]]; then
        wget -O /tmp/amneziawg-go.tar.gz "$awg_go_url"
        tar -xzf /tmp/amneziawg-go.tar.gz -C /tmp/
        
        # 找到執行檔並安裝
        find /tmp -name "amneziawg-go" -type f -executable | head -1 | xargs -I {} cp {} /usr/local/bin/awg-go
        chmod +x /usr/local/bin/awg-go
        
        # 建立相容性符號連結
        ln -sf /usr/local/bin/awg-go /usr/local/bin/awg
        ln -sf /usr/bin/wg /usr/local/bin/awg-quick
        
        log "INFO" "AmneziaWG userspace 版本安裝完成"
    else
        log "WARN" "無法下載 AmneziaWG userspace 版本，使用標準 WireGuard"
        ln -sf /usr/bin/wg /usr/local/bin/awg
        ln -sf /usr/bin/wg-quick /usr/local/bin/awg-quick
    fi
}
...
setup_amneziawg_server() {
    ...
    # 檢查是否使用核心模組或 userspace
    local use_kernel_module=false
    if lsmod | grep -q "amneziawg"; then
        use_kernel_module=true
        log "INFO" "使用 AmneziaWG 核心模組"
    else
        log "INFO" "使用 AmneziaWG userspace 模式"
    fi
...
AWG_JMIN=${AWG_JMIN}
AWG_JMAX=${AWG_JMAX}
USE_KERNEL_MODULE=${use_kernel_module}
EOF
...
EOF
    ...
    log "INFO" "systemd 服務設定完成，使用 ${wg_service}"
}
...
create_client_config() {
    ...
    # 檢查使用哪種工具生成密鑰
    local genkey_cmd="wg genkey"
    local pubkey_cmd="wg pubkey"
    local genpsk_cmd="wg genpsk"
    
    if command -v awg >/dev/null 2>&1; then
        genkey_cmd="awg genkey"
        pubkey_cmd="awg pubkey"
        genpsk_cmd="awg genpsk"
    fi
    ...
    client_private_key=$($genkey_cmd)
    client_public_key=$(echo "$client_private_key" | $pubkey_cmd)
    client_psk=$($genpsk_cmd)
...
EOF
    ...
    if lsmod | grep -q "amneziawg"; then
        log "INFO" "請使用 AmneziaWG 客戶端連接"
    else
        log "INFO" "運行於 userspace 模式，可使用 AmneziaWG 或標準 WireGuard 客戶端"
    fi
}
...
start_all_services() {
    ...
    log "INFO" "所有服務啟動完成，使用 ${wg_service}"
}
...
main() {
    ...
    if final_verification; then
        local wg_service="wg-quick"
        local awg_mode="userspace"
        if lsmod | grep -q "amneziawg"; then
            wg_service="awg-quick"
            awg_mode="kernel module"
        fi
        ...
        echo "   • AmneziaWG 模式：${awg_mode}"
        echo "   • WireGuard 服務：${wg_service}"
        ...
        echo ""
        echo "🔍 狀態檢查："
        echo "   • systemctl status ${wg_service}@${WG_IF}"
        echo "   • systemctl status warp-netns.service"
        echo ""
        echo "⚠️  重要提醒："
        if [[ "$awg_mode" == "kernel module" ]]; then
            echo "   • 客戶端必須使用 AmneziaWG 客戶端，不能使用標準 WireGuard"
            echo "   • Magic Headers 參數客戶端和伺服器必須完全一致"
        else
            echo "   • 運行於 userspace 模式，可使用 AmneziaWG 或標準 WireGuard 客戶端"
            echo "   • 若要完整 DPI 保護，建議使用 AmneziaWG 客戶端"
        fi
